...
if IsAddOnLoaded("ArkInventory") then
    ...
    if ( ArkInventory.API.Version( ) ) < 30821 then
        ...
        function ArkInventoryItemButton_CIMIUpdateIcon(self)
            ...
            CIMI_SetIcon(self, ArkInventoryItemButton_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink, bag, slot))
        end
        ...
        function CIMI_ArkInventoryUpdate(loc_id, bag_id, slot_id)
            ...
            for i=1,NUM_CONTAINER_FRAMES do
                for j=1,FoxyFarmhelper.MAX_CONTAINER_ITEMS do
                    ...
                    if frame then
                        ArkInventoryItemButton_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
                    end
                end
            end
            -- Player Bank
            for i=1,12 do
                for j=1,200 do
                    ...
                    if frame then
                        C_Timer.After(.1, function() ArkInventoryItemButton_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay) end)
                    end
                end
            end
            -- Guild Bank
            for i=1,12 do
                for j=1,200 do
                    ...
                    if frame then
                        ...
                        C_Timer.After(.2, function() ArkInventoryItemButton_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay) end)
                    end
                end
            end
        end
        ...
        FoxyFarmhelper:RegisterMessage("ResetCache", CIMI_ArkInventoryUpdate)
        ...
        hooksecurefunc(FoxyFarmhelper.frame, "ItemOverlayEvents", CIMI_ArkInventoryEvents)

        -- Makes sure things are updated if bags are open quickly after logging in. Won't always work, but better than nothing.
        C_Timer.After(15, function() FoxyFarmhelper:ResetCache() end)
        ...
        function CIMI_ArkInventoryAddFrame(frame,loc_id)
            -- Add to item frame
            -- Added a C_Timer, since FoxyFarmhelperOptions aren't loaded yet.
