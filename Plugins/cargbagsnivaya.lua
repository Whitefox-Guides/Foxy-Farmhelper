...
if IsAddOnLoaded("cargBags_Nivaya") then
    ...
    function CargBagsNivaya_CIMIUpdateIcon(self)
        ...
        CIMI_SetIcon(self, CargBagsNivaya_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(nil, bag, slot))
    end
        ...
        local function LoadFrames()
            ...
            for i=0,NUM_CONTAINER_FRAMES do
                for j=1,FoxyFarmhelper.MAX_CONTAINER_ITEMS do
                    local frame = _G["NivayaSlot"..(i*FoxyFarmhelper.MAX_CONTAINER_ITEMS+j)]
                    if frame then
    ...
    FoxyFarmhelper.frame:AddEventFunction(CIMI_CargBagsNivayaAddFrame)
    ...
    function CIMI_CargBagsNivayaUpdate(self, event, ...)
        ...
        for i=0,NUM_CONTAINER_FRAMES do
            for j=1,FoxyFarmhelper.MAX_CONTAINER_ITEMS do
                local frame = _G["NivayaSlot"..(i*FoxyFarmhelper.MAX_CONTAINER_ITEMS+j)]
                if frame then
                    C_Timer.After(.1, function() CargBagsNivaya_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay) end)
                end
            end
        end
    end
    ...
    FoxyFarmhelper.frame:AddOverlayEventFunction(CIMI_CargBagsNivayaEvents)
end
