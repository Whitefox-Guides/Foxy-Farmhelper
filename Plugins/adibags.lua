...
if IsAddOnLoaded("AdiBags") then
    ...
    function AdiBagsItemButton_CIMIUpdateIcon(self)
        ...
        CIMI_SetIcon(self, AdiBagsItemButton_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(nil, bag, slot))
    end
    ...
    FoxyFarmhelper.frame:AddEventFunction(CIMI_AdiBagsAddFrame)
    ...
    function CIMI_AdiBagsEvents(self, event, ...)
        ...
        for i=1,600 do
            ...
            if frame then
                ...
                C_Timer.After(.5, function() AdiBagsItemButton_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay) end)
            end
        end
    end
    hooksecurefunc(FoxyFarmhelper.frame, "ItemOverlayEvents", CIMI_AdiBagsEvents)

    function AdiBags_UpdateAfter()
        C_Timer.After(.5, function() CIMI_AdiBagsAddFrame(nil, "PLAYER_LOGIN") end)
        C_Timer.After(.5, function() FoxyFarmhelper.frame:ItemOverlayEvents("BAG_UPDATE") end)
    end
    LibStub('ABEvent-1.0').RegisterMessage("FoxyFarmhelper", "AdiBags_BagOpened", AdiBags_UpdateAfter)
    LibStub('ABEvent-1.0').RegisterMessage("FoxyFarmhelper", "AdiBags_ForceFullLayout", AdiBags_UpdateAfter)
end