...
if IsAddOnLoaded("Bagnon") then
    ...
    function BagnonItemButton_CIMIUpdateIcon(self)
        ...
        -- For cached Bagnon bags, GetContainerItemLink(bag, slot) would not work in FoxyFarmhelper:GetTooltipText(nil, bag, slot).
        ...
        if itemLink or cached or self:GetParent().__name == "BagnonGuildItem" then
            CIMI_SetIcon(self, BagnonItemButton_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
        else
            CIMI_SetIcon(self, BagnonItemButton_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink, bag, slot))
        end
    end

    function CIMI_BagnonUpdate(self)
        CIMI_AddToFrame(self, BagnonItemButton_CIMIUpdateIcon)
        BagnonItemButton_CIMIUpdateIcon(self.FoxyFarmhelperOverlay)
    end

    hooksecurefunc(Bagnon.Item, "Update", CIMI_BagnonUpdate)
    FoxyFarmhelper:RegisterMessage("ResetCache", function () Bagnon.Frames:Update() end)
