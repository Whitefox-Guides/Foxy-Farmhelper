...
if IsAddOnLoaded("ElvUI") then
    ...
    function ElvUI_CIMIUpdateIcon(self)
        ...
        CIMI_SetIcon(self, ElvUI_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(nil, bag, slot))
    end
    ...
    FoxyFarmhelper.frame:AddEventFunction(CIMI_ElvUIAddFrame)
    ...
    function CIMI_ElvUIUpdate(self, event, ...)
        ...
        for i=0,NUM_CONTAINER_FRAMES do
            for j=1,FoxyFarmhelper.MAX_CONTAINER_ITEMS do
                local frame = _G["ElvUI_ContainerFrameBag"..i.."Slot"..j]
                if frame then
                    ElvUI_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
                end
            end
        end
        -- Main Bank
    ...
    FoxyFarmhelper.frame:AddOverlayEventFunction(CIMI_ElvUIEvents)
end