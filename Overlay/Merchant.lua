...
function MerchantFrame_CIMIUpdateIcon(self)
    ...
    if itemLink == nil then
        ...
        CIMI_SetIcon(self, MerchantFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
    end
end
...
function MerchantFrame_CIMIOnClick()
    for i=1,MERCHANT_ITEMS_PER_PAGE do
        ...
        if frame then
            MerchantFrame_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
        end
    end
end
...
FoxyFarmhelper.frame:AddEventFunction(HookOverlayMerchant)
...
FoxyFarmhelper.frame:AddOverlayEventFunction(MerchantOverlayEvents)

FoxyFarmhelper:RegisterMessage("OptionUpdate", MerchantFrame_CIMIOnClick)
