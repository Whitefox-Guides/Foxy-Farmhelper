...
function CIMI_UpdateTradeSkillIcons(_, elapsed)
    if not FoxyFarmhelper.FrameShouldUpdate("TradeskillOverlay", elapsed or 1) then return end
    ...
    for i, button in ipairs(buttons) do
        ...
        if recipeInfo then
            ...
            if itemLink ~= nil then
                local icon = FoxyFarmhelper:GetIconText(itemLink)
...
FoxyFarmhelper.frame:AddEventFunction(TradeSkillEvents)

FoxyFarmhelper:RegisterMessage("OptionUpdate", TradeSkillEvents)
