...
local function QuestFrameUpdateIcon(self)
    ...
    if itemLink == nil then
        ...
        CIMI_SetIcon(self, QuestFrameUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
    end
end
    ...
    if rewardButtons then
        for _, rewardButton in ipairs(rewardButtons) do
            if rewardButton then
                ...
                if rewardButton.FoxyFarmhelperOverlay then
                    AddIndexInfoToCIMIFrame(rewardButton.FoxyFarmhelperOverlay, rewardButton)
                    QuestFrameUpdateIcon(rewardButton.FoxyFarmhelperOverlay)
                end
            end
        end
    end
...
FoxyFarmhelper.frame:AddEventFunction(HookOverlayQuest)
...
FoxyFarmhelper.frame:AddOverlayEventFunction(QuestOverlayEvents)
hooksecurefunc("QuestLogPopupDetailFrame_Update", QuestOverlayEvents)

FoxyFarmhelper:RegisterMessage("OptionUpdate", QuestOverlayEvents)
