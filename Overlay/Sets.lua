...
function CIMI_AddToFrameSets(parentFrame)
    ...
    if parentFrame and not parentFrame.FoxyFarmhelperOverlay then
        ...
        frame.FoxyFarmhelperSetText = frame:CreateFontString("CIMIOverlayFrame_"..tostring(parentFrame:GetName()), "OVERLAY", "GameFontNormalSmall")
        frame.FoxyFarmhelperSetText:SetPoint("BOTTOMRIGHT", -2, 2)

        function frame:UpdateText()
            if FoxyFarmhelperOptions["showSetInfo"] then
                frame.FoxyFarmhelperSetText:SetText(FoxyFarmhelper:GetSetsVariantText(parentFrame.setID) or "")
            else
                frame.FoxyFarmhelperSetText:SetText("")
            end
        end
    end
end
...
function WardrobeCollectionFrame_CIMIOnValueChanged(_, elapsed)
    ...
    if not FoxyFarmhelper.FrameShouldUpdate("WardrobeSets", elapsed or 1) then return end
    ...
    for i = 1, #setFrames do
        ...
        if frame and frame.FoxyFarmhelperOverlay then
            -- update frame
            frame.FoxyFarmhelperOverlay:UpdateText()
        end
    end
end
...
FoxyFarmhelper.frame:HookScript("OnEvent", function (self, event, addonName)
    if event == "ADDON_LOADED" and addonName == "Blizzard_Collections" then
        ...
        FoxyFarmhelper:RegisterMessage("OptionUpdate", function () C_Timer.After(.25, WardrobeCollectionFrame_CIMIOnValueChanged) end)
    end
