...
--     end
-- end
-- FoxyFarmhelper.frame:HookScript("OnUpdate", iconOverlayUpdateDelay)
...
function CIMI_CheckOverlayIconEnabled()
    ...
    if not FoxyFarmhelperOptions["showItemIconOverlay"] then
        return false
    end
    return true
end
...
function CIMI_SetIcon(frame, updateIconFunc, text, unmodifiedText)
    -- Sets the icon based on the text for the FoxyFarmhelperOverlay on the given frame.
    ...
    elseif text == "" then
        ...
        local icon = FoxyFarmhelper.tooltipOverlayIcons[unmodifiedText]
...
function CIMI_AddToFrame(parentFrame, updateIconFunc, frameSuffix, overrideIconLocation)
    ...
    if parentFrame and not parentFrame.FoxyFarmhelperOverlay then
        ...
        parentFrame.FoxyFarmhelperOverlay = frame
        ...
        local iconLocation = overrideIconLocation or FoxyFarmhelperOptions["iconLocation"]
        frame.CIMIIconTexture:SetPoint(unpack(FoxyFarmhelper.ICON_LOCATIONS[iconLocation]))
...
FoxyFarmhelper.frame:AddEventFunction(HookItemOverlay)
...
FoxyFarmhelper.frame.itemOverlayEventFunctions = {}

function FoxyFarmhelper.frame:ItemOverlayEvents(event, ...)
    if not CIMIEvents[event] then return end
    for i, func in ipairs(FoxyFarmhelper.frame.itemOverlayEventFunctions) do
        func(event, ...)
    end
end

function FoxyFarmhelper.frame:AddOverlayEventFunction(func)
    -- Adds the func to the list of functions that are called for overlay events.
    table.insert(FoxyFarmhelper.frame.itemOverlayEventFunctions, func)
end
...
function FoxyFarmhelper.FrameShouldUpdate(timerName, elapsed)
