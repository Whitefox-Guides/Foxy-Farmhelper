...
local function BlackMarketFrame_OnUpdate(self)
    ...
    CIMI_SetIcon(self, BlackMarketFrame_OnUpdate, FoxyFarmhelper:GetTooltipText(itemLink))
end
...
local function UpdateBlackMarketButtons(self, elapsed)
    if not FoxyFarmhelper.FrameShouldUpdate("BMAHOverlay", elapsed or 1) then return end
    for i=1, FoxyFarmhelper.NUM_BLACKMARKET_BUTTONS do
...
FoxyFarmhelper.frame:AddEventFunction(OnBlackMarketShow)
...
FoxyFarmhelper.frame:AddEventFunction(OnBlackMarketUpdate)
FoxyFarmhelper:RegisterMessage("OptionUpdate", function () OnBlackMarketUpdate("BLACK_MARKET_ITEM_UPDATE") end)
