...
local function GetAuctionHouseItemLink(auctionHouseButton)
    ...
    if rowData then
        if rowData.appearanceLink then
            ...
            if sourceID then
                return FoxyFarmhelper:GetItemLinkFromSourceID(sourceID)
            else
...
function AuctionHouseFrame_CIMIUpdateIcon(self)
    ...
    if itemLink == nil then
        ...
        CIMI_SetIcon(self, AuctionHouseFrame_CIMIUpdateIcon, FoxyFarmhelper.CANNOT_DETERMINE, FoxyFarmhelper.CANNOT_DETERMINE)
    else
        CIMI_SetIcon(self, AuctionHouseFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
    end
end
...
function AuctionHouseFrame_CIMIOnValueChanged(_, elapsed)
    ...
    if not FoxyFarmhelper.FrameShouldUpdate("AuctionHouseOverlay", elapsed or 1) then return end
    ...
    for i, button in pairs(buttons) do
        ...
        if frame then
            ...
            AuctionHouseFrame_CIMIUpdateIcon(button.FoxyFarmhelperOverlay)
        end
    end
end
...
FoxyFarmhelper.frame:AddEventFunction(HookOverlayAuctionHouse)
...
FoxyFarmhelper.frame:AddEventFunction(AuctionHouseUpdateEvents)

FoxyFarmhelper:RegisterMessage("OptionUpdate", AuctionHouseUpdateEvents)
