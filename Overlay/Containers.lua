...
function ContainerFrameItemButton_CIMIUpdateIcon(self)
    ...
    CIMI_SetIcon(self, ContainerFrameItemButton_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(nil, bag, slot))
end
...
function GuildBankFrame_CIMIUpdateIcon(self)
    ...
    CIMI_SetIcon(self, GuildBankFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
end
...
function VoidStorageFrame_CIMIUpdateIcon(self)
    ...
    local voidSlot = buttonSlot + (FoxyFarmhelper.NUM_VOID_STORAGE_FRAMES * (page - 1))
    local itemLink = GetVoidItemHyperlinkString(voidSlot)
    CIMI_SetIcon(self, VoidStorageFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
end
...
function VoidStorageFrame_CIMIOnClick()
    for i=1,FoxyFarmhelper.NUM_VOID_STORAGE_FRAMES do
        local frame = _G["VoidStorageStorageButton"..i]
        if frame then
            VoidStorageFrame_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
        end
    end
end
...
local function HookOverlayContainers(event)
    ...
    for i=1,NUM_CONTAINER_FRAMES do
        for j=1,FoxyFarmhelper.MAX_CONTAINER_ITEMS do
...
FoxyFarmhelper.frame:AddEventFunction(HookOverlayContainers)
...
FoxyFarmhelper.frame:AddEventFunction(OnGuildBankOpened)
...
FoxyFarmhelper.frame:AddEventFunction(OnVoidStorageOpened)
    ...
    if event == "BAG_UPDATE" then
        for i=1,NUM_CONTAINER_FRAMES do
            for j=1,FoxyFarmhelper.MAX_CONTAINER_ITEMS do
                local frame = _G["ContainerFrame"..i.."Item"..j]
                if frame then
                    ContainerFrameItemButton_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
                end
            end
        end
        ...
        for i=1,NUM_BANKGENERIC_SLOTS do
            local frame = _G["BankFrameItem"..i]
            if frame then
                C_Timer.After(0, function() ContainerFrameItemButton_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay) end)
            end
        end
    end
        ...
        if guildBankFrame then
            for column = 1, #guildBankFrame.Columns do
                for button = 1, #guildBankFrame.Columns[column].Buttons do
                    local frame = guildBankFrame.Columns[column].Buttons[button]
                    if frame then
                        C_Timer.After(.1, function () GuildBankFrame_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay) end)
                    end
                end
            end
        end
...
FoxyFarmhelper.frame:AddOverlayEventFunction(ContainersOverlayEvents)
...
FoxyFarmhelper:RegisterMessage("OptionUpdate", function () ContainersOverlayEvents("BAG_UPDATE") end)
