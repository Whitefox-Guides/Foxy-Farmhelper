...
function LootFrame_CIMIUpdateIcon(self)
    ...
    CIMI_SetIcon(self, LootFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
end
...
local function HookOverlayLoot(event)
    ...
    for i=1,FoxyFarmhelper.NUM_GROUP_LOOT_FRAMES do
...
FoxyFarmhelper.frame:AddEventFunction(HookOverlayLoot)
...
local function LootOverlayEvents(event, ...)
    for i=1,FoxyFarmhelper.NUM_GROUP_LOOT_FRAMES do
        local frame = _G["GroupLootFrame"..i].IconFrame
        if frame then
            LootFrame_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
        end
    end
end

FoxyFarmhelper.frame:AddOverlayEventFunction(LootOverlayEvents)
...
local function ChatMessageLootEvent(event, message, _, _, _, target)
    ...
    FoxyFarmhelper.cache:RemoveItem(link)

end

FoxyFarmhelper.frame:AddEventFunction(ChatMessageLootEvent)

FoxyFarmhelper:RegisterMessage("OptionUpdate", ChatMessageLootEvent)
