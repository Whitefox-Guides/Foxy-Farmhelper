...
function EncounterJournalFrame_CIMIUpdateIcon(self)
    ...
    CIMI_SetIcon(self, EncounterJournalFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
end
...
local function EncounterJournalFrame_CIMISetLootButton(self)
    ...
    local overlay = self.FoxyFarmhelperOverlay
    ...
    CIMI_SetIcon(overlay, EncounterJournalFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
end
...
function EncounterJournalFrame_CIMIOnValueChanged(_, elapsed)
    if not FoxyFarmhelper.FrameShouldUpdate("EncounterJournal", elapsed or 1) then return end
    ...
    for i = 1, #lootItemFrames do
        ...
        if frame then
            ...
            EncounterJournalFrame_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
        end
    end
end
...
FoxyFarmhelper.frame:AddEventFunction(OnEncounterJournalLoaded)
...
FoxyFarmhelper.frame:AddOverlayEventFunction(EncounterJournalOverlayEvents)

FoxyFarmhelper:RegisterMessage("OptionUpdate", EncounterJournalOverlayEvents)
