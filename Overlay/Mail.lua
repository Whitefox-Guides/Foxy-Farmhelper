...
function MailFrame_CIMIUpdateIcon(self)
    ...
    local messageIndex;
    for i=1,FoxyFarmhelper.NUM_MAIL_INBOX_ITEMS do
    ...
    CIMI_SetIcon(self, MailFrame_CIMIUpdateIcon, FoxyFarmhelper:GetTooltipText(itemLink))
end
...
function MailFrame_CIMIOnClick()
    for i=1,ATTACHMENTS_MAX_RECEIVE do
        ...
        if frame then
            MailFrame_CIMIUpdateIcon(frame.FoxyFarmhelperOverlay)
        end
    end
end
...
FoxyFarmhelper.frame:AddEventFunction(HookOverlayMail)
...
FoxyFarmhelper:RegisterMessage("OptionUpdate", MailFrame_CIMIOnClick)
